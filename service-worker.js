...
self.addEventListener("fetch", e=>{
  ...
  if(url.origin===self.location.origin){
    e.respondWith(caches.match(req).then(c=>c||fetch(req).then(r=>{ if(r&&r.ok){ const cp=r.clone(); caches.open(VERSION).then(c2=>c2.put(req,cp)); } return r; }).catch(()=>c)));
  }
});
